...
import { getGlobalPdfVectorDB } from './retrieverService.js';
import { StreamResponse, SearchResult, RelevantDocument } from '../types/index.js';
...
// 增强的论文生成函数，集成检索结果
export async function generatePaperWithRetrieval(message: string, res: any): Promise<void> {
    let isEnded = false;
    let retrievalContext = '';
    
    try {
        // 1. 首先进行文档检索
        const globalPdfVectorDB = getGlobalPdfVectorDB();
        if (globalPdfVectorDB) {
            ...
            try {
                ...
                if (results.length > 0) {
                    const relevantDocs: RelevantDocument[] = results.map((result: SearchResult, index: number) => {
                    ...
                    // 构建检索上下文
                    retrievalContext = relevantDocs.map((doc: RelevantDocument) =>
